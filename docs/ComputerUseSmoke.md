...
- BioAgent project tool smoke: `POST /api/bioagent/tools/run` 由 workspace service 执行 Scenario skill-domain 工具，文献证据评估返回 PubMed `paper-list`，结构探索返回 RCSB / AlphaFold DB `structure-summary`。
...
## 路线
- 打开 BioAgent 首页，确认研究概览、Scenario Presets、Settings、Scenario 工作台可访问。
...
- 确认 handoff 后目标 Agent 自动触发 BioAgent project tool run：知识库 `knowledge-graph` -> 文献证据评估 Scenario 自动提交 `TP53 clinical trials，返回 paper-list JSON artifact、claims、ExecutionUnit。`，并返回 PubMed 5 条记录与 `done` ExecutionUnit。

## 发现
- 2026-04-20 Python-first structure smoke：direct project tool 在 `/tmp/bioagent-python-first-smoke` 运行 prompt `帮我搜索一下pdb数据库上最新的蛋白结构，下载并在右侧栏展示其3d结构`，返回 `latest released PDB entry 10NU (2026-04-15)`，dataRef `https://files.rcsb.org/download/10NU.cif`，coordinateRef `.bioagent/structures/10NU.cif`，taskCodeRef `.bioagent/tasks/structure-abf24c5abdea.py`，atomCoordinates=195，ExecutionUnit 为 `RCSB.search.latest` / `RCSB.core.entry` / `RCSB.files.download`，均记录 `language=python`、stdout/stderr/output refs。确认没有使用默认 7BZ5 或 demo 结构。
...
- 修复前通用 AgentServer 直接 smoke 成功，但 BioAgent 文献证据评估 Scenario 的结构化长 prompt 会持续输出大量 `usage-update`，取消前 usage 超过 `135k` total tokens，未稳定收束为 `paper-list` artifact。
...
- 2026-04-20 T018 Safari Computer Use smoke：在 `http://127.0.0.1:5173/?smoke=1`、workspace `/tmp/bioagent-results-smoke` 的组学差异分析 Scenario 中运行大矩阵 prompt。默认选择 Scanpy 时动态结果区显示 `tool=scanpy.rank_genes_groups · done`、80 volcano points、8 UMAP points、12 x 8 heatmap；显式 `runner=edgeR` 后动态结果区刷新为新的 `.bioagent/omics/...json`，显示 `tool=edgeR · done`、80 volcano points、8 UMAP points、12 x 8 heatmap。
...
- handoff 自动运行已接入：`handleArtifactHandoff` 写入目标会话后设置 `handoffAutoRun`，ChatPanel 在目标 Agent 激活后消费请求并触发 BioAgent project tool，失败时再回退 AgentServer stream。复验后文献证据评估 Scenario 显示 `PubMed returned 5 paper records for: TP53 clinical trials`，ExecutionUnit params 包含 query/retmax，状态 `done`。
- 生物医学知识图谱 Scenario 已从 demo-ready 升级为 BioAgent project tool 优先；浏览器 prompt `TP53 gene include clinical trials...` 返回 UniProt reviewed human accession `P04637`、source URL、访问日期、节点与边，ExecutionUnit 为 `UniProt.uniprotkb.search` / `done`。ChEMBL / OpenTargets 仍待真实 native tool 接入。
...
## 后续固定检查
- 每次真实 BioAgent project tool 能力变更后，至少跑文献证据评估 Scenario 和结构探索 Scenario 各一次浏览器 prompt。
