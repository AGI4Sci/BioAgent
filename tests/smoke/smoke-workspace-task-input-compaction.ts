...
const taskInput = buildWorkspaceTaskInput({
  ...
  priorAttempts: Array.from({ length: 20 }, (_, index) => ({
    ...
    stderrRef: `.sciforge/logs/${index}.stderr.log`,
    failureReason: largeText,
  })),
}, {
  workspacePath: '/tmp/workspace',
  taskCodeRef: '.sciforge/tasks/run.py',
  inputRef: '.sciforge/task-inputs/run.json',
...
assert.equal(taskInput.inputRef, '.sciforge/task-inputs/run.json');
assert.equal(taskInput._sciforgeInputManifest.schemaVersion, 'sciforge.task-input.v1');
...
assert.equal(artifact.dataRef, '.sciforge/artifacts/report.json');
