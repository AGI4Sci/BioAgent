...
const workspace = await mkdtemp(join(tmpdir(), 'bioagent-failed-payload-repair-'));
    ...
    data: {
      run: {
        ...
        output: {
          result: {
            taskFiles: [{
              path: '.bioagent/tasks/generic-failing-task.py',
              language: 'python',
              content: badTask,
            }],
            entrypoint: {
            ...
            validationCommand: 'python .bioagent/tasks/generic-failing-task.py <input> <output>',
