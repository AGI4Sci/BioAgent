...
Capability Discovery 也必须进入同一条 agent 行为链路。当前状态为 partial backend retry consumption + ledger replay refs + minimal UI summary / blocked-on-full-UI-wiring-and-browser-validation，目标设计见 [`CapabilityDiscovery.md`](CapabilityDiscovery.md)：SciForge 不应把“什么时候检索能力”硬编码成一组固定触发点；初始 context 只暴露极简 `capability_discovery` API brief，AgentServer/backend 在能力不足、provider 失败或验证/修复需要换路时主动调用 discovery。Runtime/harness 只负责 API contract、审计、预算/权限和 provider-first 兜底。当前已有 contract/service/manifest/tiny brief/generated-task helper bridge、AgentServer stream-side tool-call -> tool-result bridge、session-bundle audit record、session-bundle ledger event、单向 stream 无终态时的 bounded retry handoff consumption，以及 `ProjectionApi.getCapabilityPlanSummary` 最小用户摘要；还需补齐默认 Results UI 接线/debug folding 和真实 browser 验收。
...
### Capability Discovery 原子能力（partial backend retry consumption + ledger replay refs + minimal UI summary / blocked-on-full-UI-wiring-and-browser-validation）
