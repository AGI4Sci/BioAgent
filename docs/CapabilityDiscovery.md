...
状态：partial AgentServer stream transport / blocked-on-backend-result-consumption（2026-05-17）。当前代码已经提供 `capability_discovery.search/expand/plan/explain` 的 TypeScript contract 与 runtime service、核心 manifest、AgentServer tiny handoff brief、generated-task authoring guidance、generated-task helper 调用桥、AgentServer stream-side tool-call -> tool-result bridge、session-bundle audit record 持久化和 targeted tests。生成任务可通过 `invoke_capability(task_input, "capability_discovery.search|expand|plan|explain", input)` 做 bounded progressive disclosure；AgentServer stream 若发出 `capability_discovery.*` tool-call，Gateway 会执行 discovery、发出 `tool-result`、写入 sanitized audit record，并保持 `completionEvidence=not-evidence`。剩余缺口是确认/实现 backend 对该 `tool-result` 的双向消费协议、workspace ledger 级 discovery audit refs，以及 UI `CapabilityPlanSummary`。
...
- Gateway 已能把 AgentServer stream 中的 `capability_discovery.*` tool-call 事件化为 runtime call 并 emit `tool-result`；但还需要确认/补齐 backend 对该 result 的双向消费协议，避免只在 UI/runtime audit 中可见而不能继续影响 backend 推理。
- `capability_discovery` core manifest、generated-task helper bridge 和 stream-side tool-result bridge 已落地；但不能只因 manifest/tool-result 可见就判定所有 backend 路径都已闭环消费 discovery。
...
3. [x] Add Gateway stream-side invocation transport / AgentServer tool-call surface; [ ] confirm backend bidirectional result consumption.
