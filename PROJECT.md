...
- [`docs/CapabilityDiscovery.md`](docs/CapabilityDiscovery.md)：partial AgentServer stream transport / blocked-on-backend-result-consumption 的层次化能力检索与集成模块，定义 agent-callable discovery API、分层揭示、审计和实现任务。
...
- Capability Discovery 必须成为 agent 可调用的原子能力（partial AgentServer stream transport / blocked-on-backend-result-consumption）：初始 context 只暴露极简 API brief；agent 自主调用 `search/expand/plan/explain` 做分层揭示；runtime/harness 只负责预算、权限、审计、no-secret/no-endpoint leakage 和 provider-first preflight 兜底。当前已有 contract/service/manifest/tiny brief/generated-task helper bridge、AgentServer stream-side tool-result bridge 和 session-bundle audit records；仍需确认/补齐 backend 对 tool-result 的双向消费协议、workspace ledger discovery refs 和 UI plan summary；设计细节以 [`docs/CapabilityDiscovery.md`](docs/CapabilityDiscovery.md) 为准。
...
- 状态：partial AgentServer stream transport / blocked-on-backend-result-consumption
...
- 当前事实（P4/Orchestrator 2026-05-17）：已完成 `packages/contracts/runtime/capability-discovery.ts` contract/types、核心 `capability_discovery` manifest、`src/runtime/capability-discovery.ts` service、AgentServer context-envelope tiny brief、generation prompt tiny brief、generated-task authoring guidance、generated-task helper bridge、AgentServer stream-side tool-call -> tool-result bridge、session-bundle sanitized audit record、leakage guard 和 targeted tests；生成任务可通过 `invoke_capability(task_input, "capability_discovery.search|expand|plan|explain", input)` 调用 bounded discovery；AgentServer stream 若发出 `capability_discovery.*` tool-call，Gateway 会执行 discovery 并 emit `tool-result`；discovery recommendation/plan 标记为 `not-evidence`，不能成为任务完成证据。
- Blocker：当前不能标 done，因为还需确认/补齐 backend 对 stream-side `tool-result` 的双向消费协议，避免 result 只进入 UI/runtime audit 而不能继续驱动 backend 推理；workspace ledger/replay 级 discovery audit refs 和 UI `CapabilityPlanSummary` 也未闭环。
...
- 验证：P4 added generated-task helper invocation coverage in `node --import tsx --test src/runtime/gateway/generated-task-runner-execution-lifecycle.test.ts` 11/11；`node --import tsx --test src/runtime/capability-discovery.test.ts packages/contracts/runtime/capability-manifest.test.ts` 5/5；capability_discovery owner added AgentServer stream transport coverage in `node --import tsx --test src/runtime/gateway/agentserver-stream.test.ts` 9/9 and combined discovery/generated-task suite 25/25；`npm run typecheck` 通过。
- 用户问题：Scenario Builder 中 Skills、Tools、Artifacts、失败策略、契约、质量检查等信息太多；真正有价值的用户配置应少而专业，甚至由 SciForge 在请求后自动选择。
- 设计文档：[`docs/CapabilityDiscovery.md`](docs/CapabilityDiscovery.md)，状态为 partial AgentServer stream transport / blocked-on-backend-result-consumption；当前代码已有 contract、service、manifest、handoff tiny brief、generated-task helper bridge、AgentServer stream-side tool-result/audit bridge 和 tests，但 backend 双向消费、ledger refs 和 UI summary 尚未闭环。
...
- 验证：`node --import tsx --test src/runtime/gateway/direct-context-fast-path.test.ts src/ui/src/app/projectionApi.test.ts src/runtime/gateway/context-envelope.test.ts src/runtime/gateway/agentserver-prompts.test.ts` 通过 81/81；同批 targeted suite 通过 47/47；UI-Execution Owner 追加 `node --import tsx --test src/ui/src/app/projectionApi.test.ts src/ui/src/app/results-renderer-execution-model.test.ts src/ui/src/app/results/WorkspaceObjectPreview.test.ts src/ui/src/app/uiActionBoundary.test.ts` 通过 36/36；续跑 `node --import tsx --test src/ui/src/app/projectionApi.test.ts src/ui/src/app/results/WorkspaceObjectPreview.test.ts src/ui/src/app/uiActionBoundary.test.ts` 通过 15/15；`npm run typecheck` 通过。Codex in-app browser 仍返回 `Transport closed`；隔离 Playwright 打开 `http://127.0.0.1:5173/`，title `SciForge`，console error 0，default DOM raw term matches `[]`。
...
- 验证：`node --import tsx --test src/runtime/capability-discovery.test.ts src/runtime/gateway/context-envelope.test.ts src/runtime/gateway/agentserver-prompts.test.ts packages/contracts/runtime/capability-manifest.test.ts` 通过 29/29。`node --import tsx --test tests/smoke/smoke-capability-manifest-package-discovery.ts` 当前失败在并行 capability registry sourceCounts 期望 `3 !== 6`，非本 P2 browser 改动边界但说明当前 dirty 工作树仍有未整合 registry 变更。已追加 `DISC-20260517-P2-002`、`DISC-20260517-P2-003`、`DISC-20260517-P2-004`。
...
状态：done / original gap closed; superseded by UX-SYSTEM blocked-on-backend-result-consumption
发现者：P2
...
结论：P2 原发现的 runtime invocation surface / generated helper / audit ref 缺口已关闭。仍未关闭的是更高层 `UX-SYSTEM-TASK-20260517-capability-discovery-api` 中记录的 `blocked-on-backend-result-consumption`：需要确认/补齐 backend 对 stream-side `tool-result` 的双向消费协议、workspace ledger/replay 索引和 UI `CapabilityPlanSummary`，不能用本 P2 task 再重复登记。
...
具体下一步：`UX-SYSTEM-TASK-20260517-capability-discovery-api` 与 `UX-SYSTEM-TASK-20260517-discovery-progressive-disclosure` 当前为 partial AgentServer stream transport / blocked-on-backend-result-consumption；生成任务路径已可调用 bounded discovery，AgentServer stream-side tool-call 已可转成 runtime `tool-result` 与 session audit record，下一步必须确认/补齐 backend 对该 `tool-result` 的双向消费协议、workspace ledger/replay discovery refs 和 UI `CapabilityPlanSummary`，不能只靠 tiny brief / prompt guidance / stream audit event 验收全部后端路径。`UX-SYSTEM-TASK-20260517-ui-execution-decoupling` 当前为 partial foundation，下一步优先把 WorkspaceObjectPreview/manual preview、retry/recover/import/verify transaction 和默认 projection raw scrub 收敛到 ProjectionApi/UserActionApi。并行继续 `UX-SYSTEM-TASK-20260517-universal-chat-entry`、`UX-SYSTEM-TASK-20260517-answer-first-results-panel` 和 `UX-SYSTEM-TASK-20260517-strict-user-proxy-process`。每个进程仍使用独立端口/workspace/state/config；每个 milestone 都必须更新本文件、提交并 push GitHub、关闭上一批 sub agents，再启动下一批。旧 P1-P6 strict-eval/fix/reverify 闭环已完成，可作为回归样本；未关闭的 `DISC-20260517-P5-004` 保留，但不应抢占本轮 UX 简化主线，除非它在新的真实用户评测中再次成为 blocker。
