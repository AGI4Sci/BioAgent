...
状态：partial foundation / in_progress（2026-05-17）。当前代码已经有 `ConversationProjection`、artifact delivery、workspace preview、run/audit refs、最小 `ProjectionApi` / `UserActionApi` 类型入口、UI action boundary、manual preview action 和部分 `completion-candidate` projection 读取；generated task pre-output / parse-output failure 已能产生 `completionCandidate`。但 UI 还没有整体迁移为只消费 ProjectionApi，`WorkspaceObjectPreview` 仍在组件内读取 workspace preview，通用 AgentServer handoff / `tool_payload.json` salvage import/verify transaction 尚未闭环，canonical projection scrub/conformance 也还需补齐。本文定义目标边界，不表示当前代码已经完整实现。
