...
- **User-generated Skill Library**：来自用户 workspace 的成功 task，经多次运行、自愈和验证后，由反思循环整理为 skill 草案，用户确认后安装。
...
用户请求
  ...
  → 若任务高频成功：反思循环生成新 skill 草案，用户确认后注册
```
...
| UI 架构 | 纯聊天 | 纯聊天 / 简单 Web | 混合架构（固定科学组件 + 动态编排，Scenario 自带界面） |
| 推理溯源 | 不透明 | 部分（文献引用） | 全链路（文献→推理→工具调用→结果） |
| 构建路径 | 单体应用 | 多 Agent 一体 | Scenario 先行，渐进组合 |
...
│  └──────────────────────────────────────────────────────────┘    │
├──────────────────────────────────────────────────────────────────┤
│                    Scenario 层（Phase 1 起步）                     │
│                                                                  │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐           │
│  │  文献证据评估 Scenario   │  │  结构探索 Scenario   │  │  组学差异分析 Scenario   │  ...     │
│  │ ┌──────────┐ │  │ ┌──────────┐ │  │ ┌──────────┐ │           │
│  │ │ 文献卡片  │ │  │ │ 3D查看器  │ │  │ │ 热图/UMAP │ │           │
│  │ │ PDF预览   │ │  │ │ 口袋分析  │ │  │ │ 火山图    │ │           │
│  │ └──────────┘ │  │ └──────────┘ │  │ └──────────┘ │           │
