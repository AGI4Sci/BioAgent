...
Capability Discovery 也必须进入同一条 agent 行为链路。当前状态为 partial AgentServer stream transport / blocked-on-backend-result-consumption，目标设计见 [`CapabilityDiscovery.md`](CapabilityDiscovery.md)：SciForge 不应把“什么时候检索能力”硬编码成一组固定触发点；初始 context 只暴露极简 `capability_discovery` API brief，AgentServer/backend 在能力不足、provider 失败或验证/修复需要换路时主动调用 discovery。Runtime/harness 只负责 API contract、审计、预算/权限和 provider-first 兜底。当前已有 contract/service/manifest/tiny brief/generated-task helper bridge、AgentServer stream-side tool-call -> tool-result bridge 和 session-bundle audit record；还需确认 backend 对 tool-result 的双向消费协议、ledger replay 索引和 UI plan summary。
...
### Capability Discovery 原子能力（partial AgentServer stream transport / blocked-on-backend-result-consumption）

详见 [`CapabilityDiscovery.md`](CapabilityDiscovery.md)。当前代码已有 manifest registry、capability broker、provider preflight、harness candidate projection、`invoke_capability` authoring path、`capability_discovery.search/expand/plan/explain` 的 TypeScript service、tiny handoff brief、generated-task helper、AgentServer stream-side discovery tool-call bridge、session-bundle audit record 和 targeted tests。本文只保留架构边界；API 细节、分层揭示、UI 摘要和剩余实现任务以该专项文档为准。
