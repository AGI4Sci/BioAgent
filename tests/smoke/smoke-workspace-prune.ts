...
const workspace = await mkdtemp(join(tmpdir(), 'sciforge-prune-'));
await mkdir(join(workspace, '.sciforge', 'logs'), { recursive: true });
await mkdir(join(workspace, '.sciforge', 'task-results'), { recursive: true });

await writeFile(join(workspace, '.sciforge', 'logs', 'run-a.stdout.log'), 'old log');
await writeFile(join(workspace, '.sciforge', 'task-results', 'run-a.json'), '{}');
await writeFile(join(workspace, '.sciforge', 'logs', 'run-b.stdout.log'), 'keep log');
...
assert.equal((await readdir(join(workspace, '.sciforge', 'logs'))).length, 2);
...
assert.deepEqual((await readdir(join(workspace, '.sciforge', 'logs'))).sort(), ['run-b.stdout.log']);
assert.deepEqual(await readdir(join(workspace, '.sciforge', 'task-results')), []);
